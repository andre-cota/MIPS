...
public class Functions {
	...
	static void buildFunction(String[] arguments) {
		...
		if (SwitchOp.getFormat() == "R") {
			line = functionR(SwitchOp.getOpcode(), DecForBin.switchVariable(arguments[1]), DecForBin.switchVariable(arguments[2]), DecForBin.switchVariable(arguments[3]), Main.shamt, Main.funct);
		} else if (SwitchOp.getFormat() == "I") {
			temp = Integer.parseInt(arguments[3]);
			line = functionI(SwitchOp.getOpcode(), DecForBin.switchVariable(arguments[1]), DecForBin.switchVariable(arguments[2]), DecForBin.immediateTreatment(DecForBin.binary(temp)));
		}
			
		else if (SwitchOp.getFormat() == "J")
			line = functionJ(SwitchOp.getOpcode(), DecForBin.switchVariable(arguments[1]));
		
		System.out.println("Como ficou convertido: " + line);
	}
}
